...
class MTCNNMultiSourceSLoss:
    ...
    def __call__(self, input, target, type_indicator):
        ...
        cls_loss = F.cross_entropy(cls_prob, cls_target, reduction="none")
        bbox_loss = F.mse_loss(bbox_pred, bbox_target, reduction="none")
        landmark_loss = F.mse_loss(landmark_pred, landmark_target, reduction="none")

        loss_list = (
            self.cls_weight * cls_type_indicator * cls_loss
            + self.bbox_weight * bbox_type_indicator * bbox_loss
            + self.landmark_weight * landmark_type_indicator * landmark_loss
        )
        ...
        # get top K loss mean
        topk_loss_list, _ = loss_list.topk(num_hard)
